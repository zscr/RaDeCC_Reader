...
import ast
import traceback
from radecc_main_2_2_GUI_input import radecc_reader_main
...
class App:
    ...
    def __init__ (self, master):
        self.master  = master
        self.run_complete = [False, 'Running...']
        ...
        try:
            self.logo_filename = Path.cwd()/"RR_logo.png"
            config_name = "RR_logo.png"
    ...
    def find_saved_fields_file(self, column_number, row_number):
        ...
        self.subsample_check_variable.set(self.saved_fields_file_to_load_as_df.sub_sample_check_variable[0])
    ...
    def panel1_checks (self):
        ...
        try: 
            ...
            log_file_check = pd.read_csv(self.logfile_to_load)
        except:
    ...
    def check_panel2 (self):
        ...
        if self.sample_name_column_variable.get() != 'Sample name column...':
            ...
            if self.sub_sample_option_variable.get() != 'Sub-sample name column...' and self.sub_sample_option_variable.get() != 'None':

                sub_sample_name_list = list(set(self.logfile_to_load_as_df[self.sub_sample_option_variable.get()]))
                test_list = []
                for i in range(len(self.logfile_to_load_as_df[self.sample_name_column_variable.get()])):
                    test_list.append(str(self.logfile_to_load_as_df[self.sample_name_column_variable.get()].iloc[i])+str(self.logfile_to_load_as_df[self.sub_sample_option_variable.get()].iloc[i]))
            ...
            if self.sub_sample_option_variable.get() == 'None':
                print('Linear', self.sub_sample_option_variable.get())
                logsheet_identifiers_boolean_list[1] = True
        ...
        list_of_option_variables = [self.sample_name_column_variable.get(),
                                    self.sub_sample_option_variable.get(),
    ...
    def load_panel2_entries (self):
        ...
        self.sub_sample_option_variable.set(df.sub_sample_option_variable[0])
    ...
    def run_radecc_reader_main(self):
        ...
        run_dict = {
            ...
            'number_of_blanks':[self.no_of_blanks_variable.get()],
            'number_of_detectors':[self.no_of_detectors_variable.get()],
            'th228_standard_start_activities_dict':[str(self.th228_standard_start_activity_dict)],
            'ac227_standard_manufacture_date_dict':[str(self.ac227_standard_manufacture_date_dict)],
            'ac227_standard_start_activities_dict':[str(self.ac227_standard_start_activity_dict)],
            'blank_standard_names_list':[str(self.blank_standard_names_list)],
            'detector_calibration_values_dict':[str(self.detector_calibration_values_dict)],
            'detector_adjustment_coefficient_dict':[str(self.detector_adjustment_coefficient_dict)],
            'detector_226_efficiency_dict':[str(self.detector_226_efficiency_dict)],
            'sample_name_column_variable':[self.sample_name_column_variable.get()],
            'sub_sample_option_variable':[self.sub_sample_option_variable.get()],
            'sample_mid_date':[self.sample_mid_date.get()],
            'sample_mid_time':[self.sample_mid_time.get()],
        ...
        run_df = pd.DataFrame.from_dict(run_dict, orient = 'columns')
