...
def create_effdf(output_directory, thstd, acstd, blank, ac_halfLife, 
    ...
    summary_df['E219_from_E220'] = summary_df.Average_E220*adjustment_coefficient
    summary_df['E219_from_E220_uncertainty'] = summary_df.Standard_Deviation_E220*adjustment_coefficient
    
    if os.path.exists(output_directory/'Dataframes') == False:
            os.mkdir(output_directory/'Dataframes')
    
    csv_filepath_220s = output_directory/'Dataframes'/'220_channel_efficiencies_dataframe.csv'
    if os.path.exists(csv_filepath_220s) == False:
        thstd_df.to_csv(csv_filepath_220s)
    
    csv_filepath_219s = output_directory/'Dataframes'/'219_channel_efficiencies_dataframe.csv'
    if os.path.exists(csv_filepath_219s) == False:
        acstd_df.to_csv(csv_filepath_219s)
    
    csv_filepath_blanks = output_directory/'Dataframes'/'blank_channel_efficiencies_dataframe.csv'
    if os.path.exists(csv_filepath_blanks) == False:
        blank_df.to_csv(csv_filepath_blanks)
        
    csv_filepath_summary = output_directory/'Dataframes'/'Summary_efficiencies_dataframe.csv'
